...
本节分别展示已部署 PixAgent 的工作空间、复杂商品图的功能示例与可下载的测试素材。工作空间截图拍摄于 **2026-09-17**；耳机操作截图由用户提供，保留原有「编程导航 / codefather.cn」标识。复杂场景素材于 **2026-09-19** 使用 Codex 内置生图工具生成，未调用部署实例的 DashScope / GPT 账号；它们是测试输入，不是 PixAgent 编辑后的效果或验收证据。
...
以下三张原图可直接下载并上传到 PixAgent。每张对应具体操作与观察标准，适合录制操作过程、保存前后版本并补充实际验收结果。**当前状态：素材已生成，以下场景尚未在 PixAgent 中实测。**
...
> 生图素材本身可能含文字或几何瑕疵，OCR 应以实际图中文字为准，不以生成提示词代替标准答案。真实消除、替换等模型操作需要对应 Provider 可用并可能计费；Mock 模式只能检查流程，不能验证编辑质量。
